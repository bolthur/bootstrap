...
# configure autoconf
if [ ! -f "$TARGET_COMPILE/source/autoconf-$PKG_AUTOCONF/autoconf.generated" ]; then
  # switch to source directory
  cd "$TARGET_COMPILE/source/autoconf-$PKG_AUTOCONF"
  # reconfigure
  autoreconf
  ...
  # mark as generated
  touch "$TARGET_COMPILE/source/autoconf-$PKG_AUTOCONF/autoconf.generated"
fi
...
if [ ! -f "$TARGET_COMPILE/build/autoconf-$PKG_AUTOCONF/autoconf.built" ]; then
  ...
  cd "$TARGET_COMPILE/build/autoconf-$PKG_AUTOCONF"
  # build it
  make MAKEINFO=true all -j${CPU_COUNT}
  # check for error
  if [ $? -ne 0 ]; then
    exit 1
  fi
